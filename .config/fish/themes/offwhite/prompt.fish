...
function fish_prompt
  ...
  if [ (_git_branch_name) ]
    set initial_indicator "\$ $normal->>$off_white  "
    if [ (_is_git_dirty) ]
    set initial_indicator "\$ $normal->>$red  "
  ...
  # draw prompt
  echo -n -s $user$off_white@$normal$host$off_white:$normal$cwd $off_white $initial_indicator $whitespace
end
